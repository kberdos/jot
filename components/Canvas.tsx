  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    if (e.pointerType === "touch") {
      ...
      if (touchPointers.current.size >= 2) {
        ...
        if (!pinchGesture.current || pinchGesture.current.distance === 0)
          return;
  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundPosition: `${camera.x % (30 * camera.zoom)}px ${
          camera.y % (30 * camera.zoom)
        }px`,
        touchAction: "none",
      }}
    ...
    >
      ...
      >
        ...
        {ghostSection && <GhostSectionComponent ghostSection={ghostSection} />}
      </div>
      ...
      >
        {`Camera X: ${Math.round(camera.x * 100) / 100}, Y: ${
      ...
      <button
        ...
        className={`border p-3 transition-opacity duration-500 ${
      ...
      <div className="navbar" onPointerDown={(e) => e.stopPropagation()}>
        ...
        {!isChatOpen && (
          <button className="text-xl button blue-button">Share</button>
        )}
      </div>
      ...
      {!isChatOpen && (
        <button
          ...
          className="absolute right-6 bottom-6 w-[80px] h-[80px] rounded-full bg-[var(--white)] shadow-[0_10px_30px_rgba(0,0,0,0.25)] flex items-center justify-center cursor-pointer hover:bg-[var(--grey)]"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="40"
